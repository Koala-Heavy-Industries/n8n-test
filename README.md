...
| 6. PC 登録(NetBox) | ✅ 完了 — 登録・正式化・IP・タスクの自動クローズまで実機で確認 |
| 9. 監査(weekly-audit / consistency-audit) | ✅ 完了 — やり残しレポートと実サービス突合を実機で確認 |
| 申請フォーム(メンバー・PC) | ✅ 完了 — フォーム送信 → PR → マージ → 反映を実機で確認 |
| 7・8・10・11(サーバ/VM、service-github、安全装置、本番化) | ⬜ 未着手 |

メンバーと PC の両方が「宣言・登録 → 適用 → 催促 → 完了 → 収束」まで通しで動く。

→ [実装ロードマップ](docs/design/05-environment.md#実装フェーズのロードマップ)

## 別環境で構築するには

**[docs/SETUP.md](docs/SETUP.md)** に、台帳リポジトリの作成から動作確認までの
手順をまとめてある(検証済みの順序)。台帳の雛形は `ledger-template/`。
