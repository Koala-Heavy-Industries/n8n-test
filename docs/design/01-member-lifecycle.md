...
| 申請 | 項目 |
|---|---|
| 追加 | 氏名 / メールアドレス / affiliation(従業員・協力会社+社名) / 役割 / 開始日 / sponsor・契約終了日(協力会社のみ) / サービス固有 ID(GitHub ユーザー名等) / 備考 |
| 役割変更 | 対象メールアドレス / 新しい役割 / 変更理由(必須) / 適用日(effective_from) |
...
- **差分プレビュー**: CI(bot)が PR に「この変更で 付与: … / 剥奪: …」を
  コメントする(役割マトリクスを展開)。承認者は YAML の1行差分ではなく
  権限への影響をレビューする。
- **入力検証も CI**: スキーマ・役割名の存在・affiliation 制約
  (例: `admin` は従業員のみ)・重複を CI チェックで強制する。
...
flowchart TD
    ...
    R --> M[役割マトリクスを展開し あるべき grant 集合を計算 status: left はゼロ]
    M --> D{state の付与実態と比較}
    D -->|差分なし| Z([収束済み])
...
役割に応じたグループへ追加。以降、SSO 対応 SaaS は初回ログイン時の
JIT プロビジョニングで連鎖する。GitHub 招待などの自動操作、ライセンス購入・
...
新旧役割のマトリクス差分だけが適用される(付与と剥奪の両方向)。
剥奪を含む場合は差分プレビューの警告と二重承認が効く。
