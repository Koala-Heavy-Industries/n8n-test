...
- **正式化の経路は2つ**あり、どちらを使ってもよい:
  1. **機器情報更新フォーム(device-update。PC・サーバ共通 → [07](07-servers.md))** —
     ...
     部分更新する。**登録時と同じ重複チェック(name / asset_tag)を更新時にも
     行う**(正式値が既存機器と衝突しうるため)。n8n が NetBox を更新
     (name / asset_tag の書き換え、provisional タグ除去)し、state を更新する。
     資産管理番号が変わった場合は **bot が state ファイルをリネーム(git mv)**する。
  2. **NetBox を直接更新** — remind-scheduler の事後検証(verify: `api`)が
     「provisional タグが外れて正式値が入った」ことを検知し、タスクを
     自動クローズする。**更新の経路を強制しない**のがポイント。
     この経路でも**事後検証が NetBox の値を state へ書き戻し、資産管理番号が
     変わっていればファイルをリネームする**(state を NetBox に追随させる。
     経路によって台帳の鮮度が変わらないようにする)。
...
- 固定 IP 運用のため、入力時は NetBox からの自動払い出しも利用できる
  (→ [07「IP の払い出し」](07-servers.md#ip-の払い出し))。
