...
- **楽観的排他+リトライ(主たる仕組み。実装済み)**: 読み取り時のファイル SHA を
  保持し、更新時にその SHA を指定する(GitHub Contents API)。SHA 不一致
  (他の実行が先に書いた)なら GitHub が 409 を返すので、**読み直して patch を
  再適用し、リトライ**する(上限3回)。上限超過はエラーとし、日次リコンサイルの
  再試行に委ねる。競合以外の失敗(権限不足など)は再試行せず即座に失敗させる。
...
> 直列化する」としていたが、**n8n にワークフロー単位の同時実行数の設定は無い**
> (`N8N_CONCURRENCY_PRODUCTION_LIMIT` は全ワークフロー共通のグローバル設定で、
> 1 にすると全処理が直列化されてしまう)。
> したがって直列化には依存せず、**上記の楽観的排他+リトライを正とする**。
> 規模が大きくなり競合が頻発するようなら、グローバル制限の導入を検討する。

リトライは n8n のループ辺ではなく**サブワークフローの再帰呼び出し**で実装している
(`attempt` を増やして自分自身を Execute Workflow する)。ノード間で状態を持ち回る
必要がなくなり、実行ごとにコンテキストが独立するため壊れにくい。
