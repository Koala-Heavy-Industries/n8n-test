...
- セキュリティ: n8n の resumeUrl は**署名付き**で発行される
  (`...?signature=<HMAC>` が自動で付き、改竄・生成はできない)。
  ただし **URL を知っていれば誰でも押せる**ことに変わりはないため、
  「n8n を社内ネットワーク限定で公開し、リンクは承認者・担当者本人にのみ送る」を
  前提とする。押した人の本人性が必要になったら、承認者ごとに異なるトークンを
  クエリに載せて再開後の Code ノードで検証する方式に強化できる(IF は不変)。
- **実装上の注意(検証済み)**: resumeUrl は既にクエリ(`?signature=...`)を
  含むため、`?decision=approve` を単純に連結すると URL が壊れる。
  区切り文字を判定して `&` で連結すること。
